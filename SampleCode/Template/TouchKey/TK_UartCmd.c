...
int8_t TK_CmdType1(uint8_t *pu8RXBuf)
{
    ...
    *((uint8_t *)&u32Data+3) = pu8RXBuf[1];
    ...
    case E_CMD_TYPE1_SPECIFY_FEATURE:
        S_TkFeat.u8EnIIR = pu8RXBuf[1];
        S_TkFeat.u8EnDebounce = pu8RXBuf[2];
        S_TkFeat.u8EnBaseLineTrace = pu8RXBuf[3];
        break;

    case E_CMD_TYPE1_SPECIFY_SENSE:
        psTkInfo->u8SenPulse = pu8RXBuf[1];             /* Sense Pulse Width  */
        psTkInfo->u8SenTimes = pu8RXBuf[2];             /* Sense Times  */
        psTkInfo->u8PolarityCapacitorBank = pu8RXBuf[3];    /* Specified the level for enabled channels on non-measured period */
        break;

    case E_CMD_TYPE1_SPECIFY_RESET_VOL:
        S_TkFeat.u16ResetTime = pu8RXBuf[1] | ((uint16_t)(pu8RXBuf[2]) << 8);
        psTkInfo->u8AvcchLevel = pu8RXBuf[3];
        break;
...
int8_t TK_CmdType2(uint8_t *pu8RXBuf)
{
    ...
    case E_CMD_TYPE2_READ_REF_SHIELD_NO:
        ...
        gu8TXBuf[3] = i8Percentage;
        break;

    case E_CMD_TYPE2_READ_FEATURE:
        gu8TXBuf[0] = S_TkFeat.u8EnIIR;
        gu8TXBuf[1] = S_TkFeat.u8EnDebounce;
        gu8TXBuf[2] = S_TkFeat.u8EnBaseLineTrace;
        //gu8TXBuf[3] = 0x0;
        break;
    ...
    case E_CMD_TYPE2_READ_RESET_VOL:
        gu8TXBuf[0] = S_TkFeat.u16ResetTime;
        gu8TXBuf[1] = S_TkFeat.u16ResetTime >> 8;
    ...
    case E_CMD_TYPE2_GET_CALIBRATION1:
        ...
        {
            chan &= ~0x80;
            gu8TXBuf[0] = inp8(TK_ADDR_KEY_PARM + TK_CCB_OFFSET + chan * 4);
            gu8TXBuf[1] = inp8(TK_ADDR_KEY_PARM + TK_REFCB_OFFSET + chan * 4);
        }
        else
        {
            gu8TXBuf[0] = (pKeyInfo+chan)->ccb;
            gu8TXBuf[1] = (pKeyInfo+chan)->refcb;
        }
        break;
    ...
    case E_CMD_TYPE2_GET_CALIBRATION2:
        ...
        if(chan & 0x80)
        {
            ...
            chan &= ~0x80;
            u8Level = inp8(TK_ADDR_KEY_PARM + TK_LEVEL_OFFSET + chan * 4);
            gu8TXBuf[0] = u8Level;
            u8Level = inp8(TK_ADDR_KEY_PARM + TK_THRESHOLD_OFFSET + chan * 4);
            gu8TXBuf[1] = u8Level;
        }
        else
        {
            ...
            gu8TXBuf[0] = (pKeyInfo+chan)->level;
            gu8TXBuf[1] = (pKeyInfo+chan)->threshold;
        }
        break;
...
int8_t TK_GetPacket(uint32_t* pu32ChanelMsk)
{
    ...
    *pu32ChanelMsk = (psTkInfo->u32EnChanMsk) | ((psTkInfo->u32EnSliderMsk | psTkInfo->u32EnWheelMsk)\
                     | (1<<psTkInfo->u8RefChan));

    if(psTkInfo->u8ShieldChan != 0xFF)
        *pu32ChanelMsk = *pu32ChanelMsk | (1<<psTkInfo->u8ShieldChan);
